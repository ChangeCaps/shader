...
float random21(vec2 c) {
	return fract(4096.0 * sin(dot(c, vec2(17.0, 59.4)))) * 2.0 - 1.0;
}
...
vec3 rotate(in vec3 position, in vec3 v, in vec3 u) {
...
void pass(inout Renderer renderer, in Hit hit) {
	switch (hit.material) {
		...
		case WATER:
			...
			refraction_ray.direction = normalize(mix(hit.incident, -hit.normal, dot(hit.incident, -hit.normal) * 0.4));
...
void material_shading(inout Renderer renderer, in int hit_index, in vec3 material, inout vec3 color) {
	...
	float sun_shadow = shadow(sun_shadow_ray, 16.0);	
	...
	switch (hit.material) {
		...
		case WATER:
			...
			color = mix(refraction_color, reflection_color, 0.5 + 0.5 * clamp(1.0 - exp(-0.0001 * pow(refraction_hit.distance, 3)), 0.0, 1.0));

			color *= material;

			color += 0.5 * material * vec3(1.0, 0.9, 0.9) * sun_shadow * sun_diffuse;
			color += 0.1 * material * vec3(0.3, 0.3, 0.8) * sky_diffuse;
